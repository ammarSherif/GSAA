...
//                      
        ...
        stage.setTitle("Enter the Cost");
        GridPane grid = new GridPane();
        grid.setPadding(new Insets(10, 10, 10, 10));
        grid.setVgap(5);
        grid.setHgap(5);
        Label textCost = new Label("Cost value : ");
        textCost.setStyle("-fx-font-family: \"Arial\";"
                + "-fx-font-size: 12px;"
                + "-fx-font-weight: bold");
        final TextField cost = new TextField();
        cost.setPromptText("Enter the Cost vlaue");
        cost.getText();
        GridPane.setConstraints(textCost, 0, 0);
        GridPane.setConstraints(cost, 1, 0);
        grid.getChildren().addAll(textCost, cost);
        ...
        Label comment = new Label(null);
        GridPane grid2 = new GridPane();
        grid2.setPadding(new Insets(10, 10, 10, 10));
        grid2.setVgap(5);
        grid2.setHgap(5);
        GridPane.setConstraints(comment, 0, 0);
        grid2.add(comment, 0, 0);
        grid.getChildren().addAll(enter, cancel);
        GridPane finalGrid = new GridPane();
        finalGrid.addColumn(0, grid, grid2);
        comment.setStyle("-fx-text-fill: red;");
        finalGrid.setStyle("-fx-background-color: #FFFFFF");
        Scene scene = new Scene(finalGrid, 340, 115);
        stage.setScene(scene);
        stage.setResizable(false);
        stage.show();
        cost.setOnKeyPressed((KeyEvent e) -> {
        ...
        enter.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                if (City.isDouble(cost.getText())) {
                    costLabel.textProperty().bind(cost.textProperty());
                    setCost(Double.parseDouble(cost.getText()));
                    costLabel.textProperty().unbind();
                    stage.close();
                    pressed = false;
                } else {
                    comment.setText("Error : cost must be a number");
                }
            }
        });
